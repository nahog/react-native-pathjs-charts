...
export class AxisStruct {
  ...
  static getTickValues(axis, tickCount, decimalPlaces) {
    const tickStep = AxisStruct.calcStepSize(
  ...
  axis() {
    ...
    const ticks =
      this.options.tickValues !== undefined &&
      this.options.tickValues.length !== 0
        ? _.map(this.options.tickValues, function(v) {
            return v.value;
          })
        : AxisStruct.getTickValues(currentAxis, tickInterval, decimalPlaces);
    const fixed = this.options.zeroAxis
      ? this.scale(0)
      : horizontal
        ? yAxis.min
        : xAxis.min;
    ...
    return {
      ...
      lines: ticks.map((c, i) => {
        ...
        const lineStart = {
          x: horizontal
            ? this.scale(scaleBase) + margin.left
            : xAxis.min + margin.left,
          y: horizontal
            ? yAxis.min + margin.top
            : this.scale(scaleBase) + margin.top
        };
        return Pathjs()
          .moveto(lineStart)
          .lineto(
            horizontal ? lineStart.x : xAxis.max + margin.left,
            horizontal ? yAxis.max + (margin.top - tailLength) : lineStart.y
          );
      }, this)
    };
  }
...
export default class Axis extends Component {
  render() {
    ...
    if (options.orient === "top" || options.orient === "bottom")
      textAnchor = "middle";
    ...
    const ticks = _.map(axis.ticks, function(c, i) {
      const label =
        options.labelFunction !== undefined
          ? options.labelFunction.apply(this, [c])
          : c;
      let scaleBase = isNaN(c) ? i : c;
      let gxy = horizontal
        ? [scale(scaleBase), chartArea.y.min]
        : [chartArea.x.min, scale(scaleBase)];

      let returnValue;
      if (label !== undefined && label !== null) {
        returnValue = (
          <Svg.G key={i} x={gxy[0]} y={gxy[1]}>
            {options.showTicks && (
              <Svg.Circle
                r={options.tickSize}
                cx="0"
