...
export default class PieChart extends Component {
  ...
  _shouldAnim = () =>
    this.props.options.animate.enabled && this.props.data.length > 1;
  ...
  render() {
    ...
    if (this.props.data.length === 1) {
      ...
      let stroke =
        typeof fill === "string" ? outerFill : Colors.darkenColor(outerFill);
      slices = (
        <Svg.G>
          <Svg.Circle
            r={R}
      ...
      slices = chart.curves.map((c, i) => {
        let fill =
          (c.item.color && Colors.string(c.item.color)) || this.color(i);
        ...
        return (
          <Svg.G key={i}>
            <Svg.Path
              ref={ref => (this._animationRefArray[`SLICE${i}`] = ref)}
              fill={fill}
              fillOpacity={opacity}
    ...
    if (this._shouldAnim())
      setTimeout(this._startAnimation, ANIMATION_START_DELAY);
